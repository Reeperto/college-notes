...
\begin{abstract}
    Graphs lend themselves naturally to matrices that encode their properties, such as which vertices are connected to others and how many vertices are connected to a given vertex. These matrix representations allow us to analyze graphs outside of traditional combinatoric approaches by considering their eigenvalues and eigenvectors. Key to our study is the Laplacian matrix representation of a graph. We first show that the eigenvalues of the Laplacian matrix give insight into if a graph is connected, and if not how many separate components there are in which this is the case. We then frame this result within the context of of computational geometry and discuss how more advanced problems in computational geometry can be approached with this spectral framework.
\end{abstract}

\section{Introduction}

Graphs at their core encode connectivity. A graph is simply vertices and edges where vertices are objects and edges are connections between these objects. While simple in concept, this means that anywhere there are pairwise relationships between objects, a graph structure can be assigned. We see graphs appear when modeling friendship networks on social media platforms, determining optimal flight paths, describing the structure of chemical compounds, and of importance to us analyzing meshes in computational geometry. Because connectivity is fundamental to many differing fields and applications, it is of great interest for us to develop theories and algorithms for graphs.

Graph theory as a field has always been deeply tied to combinatorics. One of the earliest problems in graph theory is Euler's famous K\"onigsberg bridge problem. In the town of K\"onigsberg, there were $7$ bridges connecting $4$ land masses. The question is if there was a possible route one could walk that would cross each bridge exactly once. Euler reframed the problem by considering the bridges as edges and each land mass as vertices. By employing the fundamental concept of counting from combinatorics, Euler solved the problem in terms of the parity of the number of edges going into a vertex for each land mass \cite{wilson2013combinatorics}. %TODO: Make connection to modern graph theory

In this paper, we will explore an alternative way of viewing and understanding graphs. We will seek to construct a bridge between the discrete graph structure and a more algebraic description. The question is what algebraic description or structure should we use? Note that if we limit the number of vertices of a graph to be finite, we can index its vertices as $v_1, \ldots, v_n$. We can then lay out in a $2$D grid all possible pairs of vertices and mark each cell when the corresponding vertices are connected. Quite naturally, this same grid could be expressed as a matrix where each row and column represents some vertex and the entries are $1$ if two vertices are connected and $0$ otherwise. We therefore will be interested in understanding such matrix representations of graphs and what we can conclude about a graph from them. This will lead us to a field of math called \term{spectral graph theory}.

In section one, we will first lay out concretely what both graphs and connectedness are as well as some basic matrix representations like the one alluded to previously. In section two, we will investigate the spectra of a certain matrix representation of a graph called the Laplacian matrix and show that the connectedness of a graph can be described by a given eigenvalue's algebraic multiplicity. In section three, we will then reframe this result as a solution to a computational geometric problem as well as look at another related problem that spectral graph theory can solve.

% We will find that such possible matrix representations of graphs will have well behaved spectra (that is well behaved eigenvalues and eigenvectors). We will then show that the connectedness of a graph is encoded in the eigenvalues of a certain matrix representation of a graph, a result we can then reframe in computational geometry.

\section{Background}

We first outline the formal structure of a graph and related definitions that will be used throughout the paper, following the formalism and notation from \cite{diestelGraphTheory2017}. We let $[S]^n$ denote the set of all $n$-element sized subsets of $S$.

\begin{definition}[Graph Structure]
    A \term{graph} is a pair $G = (V, E)$ of sets where $E \subseteq [V]^2$. The elements of $V$ are \term{vertices} and the elements of $E$ are \term{edges}. Two vertices $v_1$ and $v_2$ are \term{adjacent} or \term{neighbors} if $\qty{v_1, v_2} \in E$. We denote $\qty{v_1, v_2} \in E$ by $v_1 \sim v_2$. The \term{set of neighbors} of a vertex $v$ is denoted by $N(v) \coloneq \qty{w \in V : v \sim w}$. The \term{degree} of a vertex $v$ is $\deg(v) \coloneq |N(v)|$. A \term{subgraph} $H$ of $G$, denoted by $H \subseteq G$, is a graph whose vertex and edge sets are subsets of $G$'s.
\end{definition}
...
For the purposes of this paper, we will assume that every graph has finitely many vertices (and hence finitely many edges). Consider the illustrations of two graphs in Figure \ref{fig:graph_visual}. Notice that they differ in terms of how connected their vertices are to one other. If each vertex was a city and each edge a road, a car driving on \ref{fig:basic_graph} could get to any city, but a car on the outer ring of \ref{fig:connected_components} would be unable to get to the inner ring. That is, there is some path that the car can take from any city to any other city in \ref{fig:basic_graph} but not in \ref{fig:connected_components}. We formalize this notion of paths and connectedness as follows.
...
As previously discussed, we will attempt to gain insight about graphs and their connectivity by representing them algebraically as matrices. Of initial importance to us are the adjacency matrix and degree matrix which both capture in different ways the connectivity between vertices.
...
While the adjacency and degree matrix may offer some useful insight into a graph based on their spectra, it will turn out that combining the two into a new matrix representation will better capture the relationship between eigenvalues and the connectivity of a graph more clearly. This new matrix representation is called the \term{Laplacian matrix}.
...
We will find in further analysis more utility in an alternative description of the Laplacian matrix. If we assign a real number to each vertex of a graph with $n$ vertices, we can view this as a vector $x \in \R^n$ where each component corresponds to a vertex. Therefore the Laplacian matrix can be used to in a sense operate on $x$ like in $Lx$. This gives rise to the \emph{quadratic form} of the Laplacian. We will not provide a derivation of it here, but one can be found in \cite{mohar2004graph}.
...
\begin{definition}
    ...
    \begin{itemize}
        \item $V \subseteq \R^3$ is a finite set representing the points or vertices
        \item $E \subseteq [V]^2$ is a set representing the edges
        \item $F \subseteq [E]^3$ is a set representing the faces such that for any $f = \qty{e_1, e_2, e_3} \in F$, 
            \[
...
Notice that a triangular mesh lends itself to some very natural graph structures. One is simply using the vertices as the graph vertices and the edges as graph edges. The other one which is of use to us is using the faces as graph vertices and faces sharing a common edge as the edge set. This is often referred to as the dual mesh and appears in computational geometry problems such as Delaunay triangulations, mesh based signal processing \cite{taubinDualMeshResampling2002} and refinement of meshed implicit surfaces \cite{10.1145/566282.566308}. 

\begin{definition}
    The \term{dual mesh} of a triangular mesh $K = (V_K, E_K, F_K)$ is the graph $G = (V, E)$ such that $V = F_K$ and $f_i \sim f_j$ if $f_i \cap f_j = \qty{e}$ for some in $e \in E_K$.
\end{definition}
...
Imagine living on the surface of some face in a mesh. Then one can freely walk around the surface of that face. Furthermore, one can walk across edges to adjacent faces in a well defined manner as there can only be at most one possible choice due to the last restriction on faces in Definition \ref{def:tri_mesh}. Passing through a vertex is less well defined because there can be an arbitrary number of possible faces to go to. We will therefore define (quite informally) that a mesh is \term{connected} if any two points (starting on faces) can be connected via a path on the mesh surface that crosses between faces via edges only. In the same vein as a graph, a \term{connected component} of a mesh is a maximal submesh that is connected. This quickly lends itself to a reframing of Theorem \ref{thm:main_thm}.
...
\begin{proof}
    The notion of connectedness for meshes we constructed is captured by the connectivity of the dual mesh. Our requirement that mesh connectivity happens across edges is identical to connecting faces with a common edge as in the definition of the dual mesh. Therefore the question of how many connected components a mesh has is the same as how many connected components does its dual mesh have. Its dual mesh will have a corresponding Laplacian matrix which we know from Theorem \ref{thm:main_thm} that the algebraic multiplicity of zero equals the number of connected components. Therefore since the number of connected components in the mesh is identical to the number of connected components of its dual mesh, we have the desired if and only if.
\end{proof}

\begin{remark}
    The connectivity of a mesh could also have been defined to align with the natural graph structure induced by its points and edges. However, it is generally more useful to interpret a mesh as a discretization of some surface for application purposes. Therefore it makes sense that the faces of a mesh (that themselves can be thought of as two dimensional surfaces) better describe the mesh's actual surface and hence connectivity more so than the mesh's point and edge sets.
\end{remark}

While this result may seem obvious having seen it established in a graph theoretic context, it serves to illustrate how simple it can be to translate results from spectral graph theory to computational geometry. If one wanted to, they could use the previous result and determine how many components a mesh has by numerically finding the algebraic multiplicity of zero for its associated Laplacian matrix. This general approach we took of taking the dual, creating the Laplacian matrix of the dual, then applying some result about its spectra lends itself to some very powerful algorithms. While we were interested in a strict sense of connectivity, one may wonder if the connectivity of a graph could be represented as a more continuous quantity. This question lends itself to the realm of \term{spectral partitioning} in which a (possibly already connected mesh) is separated into submeshes that maximize some more continuous sense of connectedness. This separation is typically dictated by examining the unit norm eigenvector associated with the smallest positive eigenvalue of the Laplacian matrix (called the ``Fiedler Vector''). Spectral partitioning gives very natural groupings of a mesh making it useful in medical applications \cite{10.1007/978-3-642-15705-9_41} and mesh processing \cite{bao2023spectral} (see Figure \ref{fig:spectral_part}).

\begin{figure*}
    \centering
    \begin{subfigure}[t]{0.2\pagewidth}
        \centering
        \includegraphics[width=\linewidth]{spectral_partitioning.png}
        \caption{Mesh partitioning from \cite{bao2023spectral}}
    \end{subfigure}
    \hspace{1cm}
    \begin{subfigure}[t]{0.4\pagewidth}
        \centering
        \includegraphics[width=\linewidth]{fielder_vector_colon.png}
